...
微服务项目中需要使用一个统一的管理分布式系统的配置中心。
工程采用 Spring Cloud Config 搭建配置中心。共有三个项目，分别是：
...
spring:
  cloud:
    config:
      server:
        git:
          uri: http://192.168.2.211:8080/kakusilong/spring-config-demo.git
...
```
{
  ...
  "propertySources": [
    {
      "name": "http://192.168.2.211:8080/kakusilong/spring-config-demo.git
